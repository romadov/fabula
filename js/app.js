(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".tabs-block__slider")) {
            const tabsSwiper = new swiper_core_Swiper(".tabs-block__slider", {
            ...
            const progressBarContainer = document.querySelector(".tabs-block__progressbar");
            if (progressBarContainer && !progressBarContainer.querySelector(".swiper-pagination-progressbar-fill")) {
                const fillEl = document.createElement("div");
                fillEl.className = "swiper-pagination-progressbar-fill";
                progressBarContainer.appendChild(fillEl);
            }
            tabsSwiper.on("progress", (function() {
                const progressBarFill = document.querySelector(".tabs-block__progressbar .swiper-pagination-progressbar-fill");
                if (progressBarFill) {
                    const totalSlides = tabsSwiper.slides.length;
                    const adjustedProgress = (this.progress * (totalSlides - 1) + 1) / totalSlides;
                    progressBarFill.style.transform = `scaleX(${adjustedProgress})`;
                }
            }));
            const navButtons = document.querySelectorAll(".navigation-slider__button");
        ...
        if (document.querySelector(".projects__slider")) {
            ...
            new swiper_core_Swiper(".projects__slider", {
                modules: [ Navigation ],
    ...
    const tabsContainer = document.querySelector(".tabs-block__links");
    tabsContainer.addEventListener("click", (function(event) {
        if (event.target.classList.contains("tabs-block__link")) {
            event.preventDefault();
            tabsContainer.querySelectorAll(".tabs-block__link").forEach((link => {
...
*/    var Observer_gsap, Observer_coreInitted, Observer_win, Observer_doc, _docEl, _body, _isTouch, _pointerType, ScrollTrigger, _root, _normalizer, _eventTypes, Observer_context, _getGSAP = function _getGSAP() {
    ...
    Observer_Observer.version = "3.12.7";
...
*/
    ...
    let items = gsapWithCSS.utils.toArray(".studios__item");
    ...
    let st = gsapWithCSS.utils.toArray(".studios-prices__item");
    gsapWithCSS.set(st, {
        opacity: 0,
        y: -50
    });
    ...
    let pack = gsapWithCSS.utils.toArray(".packages__item");
    gsapWithCSS.to(pack, {
        autoAlpha: 1,
        duration: .5,
        stagger: .3,
    ...
    pack.forEach(((packs, i) => {
        let pack_num = packs.querySelector(".packages__number");
        let pack_title = packs.querySelector(".packages__subtitle");
        let pack_desc = packs.querySelector(".packages__description");
        let pack_price = packs.querySelector(".packages__price");
        let pack_order = packs.querySelector(".packages__order");
        let pack_more = packs.querySelector(".packages__more");
        gsapWithCSS.set([ pack_num, pack_title, pack_desc, pack_price, pack_order, pack_more ], {
            opacity: 0,
            x: -50
        });
        let pack_anim = gsapWithCSS.timeline({
            scrollTrigger: {
                trigger: packs,
        ...
        pack_anim.to(pack_num, {
            x: device_width < 1025 ? 0 : -95,
            opacity: 1,
            duration: 1.2
        });
        pack_anim.to(pack_title, {
            x: 0,
            opacity: 1,
            duration: 1.2
        }, "-=1");
        pack_anim.to(pack_desc, {
            x: 0,
            opacity: 1,
            duration: 1.2
        }, "-=1");
        pack_anim.to(pack_price, {
            x: 0,
            opacity: 1,
            duration: 1.2
        }, "-=1");
        pack_anim.to(pack_order, {
            x: 0,
            opacity: 1,
            duration: 1.2
        }, "-=1");
        pack_anim.to(pack_more, {
            x: 0,
            opacity: 1,
            duration: 1.2
        }, "-=1");
    }));
    let itemsFormats = gsapWithCSS.utils.toArray(".formats__item");
    gsapWithCSS.set([ itemsFormats, ".formats__title", ".formats__subtitle", ".formats__list li" ], {
        opacity: 0,
        y: 50
    });
    gsapWithCSS.to([ ".formats__title", itemsFormats, ".formats__subtitle", ".formats__list li" ], {
        opacity: 1,
        y: 0,
        duration: .7,
        stagger: .3,
    ...
    let itemsPack = gsapWithCSS.utils.toArray(".packages__info");
    gsapWithCSS.set(itemsPack, {
        opacity: 0,
        y: 100
    });
    gsapWithCSS.to(itemsPack, {
        opacity: 1,
        y: 0,
        stagger: .2,
        duration: .5,
    ...
    gsapWithCSS.set(".formats__bottom .booking", {
        scale: 0
    });
    gsapWithCSS.to(".formats__bottom .booking", {
        scale: 1,
        duration: .4,
        scrollTrigger: {
            trigger: ".formats__bottom",
            start: "top center",
            markers: false
        }
    });
    let itemsAbout = document.querySelector(".about__items");
    let itemAbout = gsapWithCSS.utils.toArray(".about__item");
    function initCustomGSAP() {
        if (window.matchMedia("(min-width: 1024px)").matches) {
            let totalWidth = itemAbout.reduce(((acc, item) => acc + item.offsetWidth + 20), 0);
            gsapWithCSS.set(itemsAbout, {
                x: 300
            });
            gsapWithCSS.to(itemsAbout, {
            ...
            itemAbout.forEach((item => {
                let rotateValue = item.dataset.rotate ? parseFloat(item.dataset.rotate) : gsapWithCSS.utils.random(-10, 10);
                let randomX = gsapWithCSS.utils.random(-10, 10);
                let randomY = gsapWithCSS.utils.random(-15, 15);
    ...
    gsapWithCSS.fromTo(".presentation-info__content > *", {
        opacity: 0,
        y: 100
    }, {
        opacity: 1,
        ...
        stagger: .2,
        scrollTrigger: {
            trigger: ".presentation-info__content",
            start: "top 80%",
            toggleActions: "play none none none"
        }
    });
    gsapWithCSS.fromTo(".presentation-info__image img", {
        scale: 0
    }, {
        scale: 1,
        duration: 1.2,
        ease: "power2.out",
        scrollTrigger: {
            trigger: ".presentation-info__image",
            start: "top 80%",
            toggleActions: "play none none none"
        }
    });
    if (window.matchMedia("(min-width: 1024px)").matches) {
        gsapWithCSS.utils.toArray(".tabs-block__item").forEach((item => {
            const elements = item.querySelectorAll(".tabs-block__title, .tabs-block__text, .tabs-block__price, .tabs-block__actions");
            elements.forEach((el => {
        ...
        gsapWithCSS.utils.toArray(".tabs-block__image-block").forEach(((card, index, array) => {
            let triggerEnd = index === array.length - 1 ? "center center" : "center top";
            ...
            if (index === array.length - 1) tl.fromTo(card, {
                rotation: -20,
                y: 50,
                opacity: 0
            }, {
